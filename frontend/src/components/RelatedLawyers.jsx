...
const RelatedLawyers = ({ speciality, docId }) => {
  ...
  useEffect(() => {
    if (lawyers.length > 0 && speciality) {
      const lawyersData = lawyers.filter((doc) => doc.speciality === speciality && doc._id !== docId)
      setRelDocs(lawyersData)
    }
  }, [lawyers, speciality, docId]) // ✅ Corrected dependencies
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      <button 
        onClick={() => { navigate('/lawyers'); scrollTo(0, 0) }} // ✅ Changed from `/doctors` to `/lawyers`
